...
class Webtex_WeddingRegistry_Block_Shipping extends Mage_Checkout_Block_Onepage_Shipping
{
    public function getCheckBoxRegistryAddress()
    {
        ...
        if(isset($webtex_weddingregistry_id) && $webtex_weddingregistry_id)
        {
            ...
            if($registry->getData('address_id'))
            {
                ...
                $answer = '<p><input type="checkbox" name="shipping[weddingregistry]" id="shippingweddingregistry" value="'.$registry->getData('address_id').'" /> &nbsp;<label for="shippingweddingregistry">';
                $answer .= $name; 
                $answer .= '</label></p>';
            }            
        }  
